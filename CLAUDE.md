...
- **Don't add consumer-flavoured strings to the bridge.** Hardcoded command names, project paths, or persona text belong in the consumer's repo. If a PR adds a real consumer name anywhere outside the `.example` templates, push back.
- **Don't ship a consumer with a tight bootstrap allow-list.** The agent can only read operator approvals after its `Monitor` is attached to `bot.stdout.log`, and attaching the Monitor is itself a tool call (same for the `Bash` / `Read` / `Write` calls the bootstrap skill issues before it). If any of those hit the permission hook on first run, the hook denies + DMs + waits for an ack — but the ack would arrive as a Telegram message that only the not-yet-attached Monitor could deliver. Deadlock. Document the consumer-side trap in the README ("Bootstrap trap"), and when helping a new consumer wire up their allow-list, insist on bare-name entries for `Monitor`, `Bash`, `Skill`, and the Task tools until the session is proven to bootstrap cleanly; tighten only once the operator has seen the tool traffic.
